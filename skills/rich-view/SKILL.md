...
presigned URL that expires after the configured retention (default 1 day,
max 7)**. The chat message carries the link; the page dies on schedule.
...
4. Send the link with a one-line summary AND the expiry notice, e.g.
   "📊 12 leads · temporary link (valid ~1 day): <url>".
...
objects after 2 days (physical cleanup; the presigned link itself stops
working at RICH_VIEW_TTL_SECONDS) → set:
...
RICH_VIEW_S3_REGION=auto     RICH_VIEW_TTL_SECONDS=86400
```
...
- The URL is unguessable but **unauthenticated** — anyone holding the link can
  ...
  keep the TTL within the 1–7 day range (SigV4 cannot exceed 7 days anyway).
